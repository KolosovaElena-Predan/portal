...
if ($checkOnly) {
    ...
    if (strlen($query) >= 2) {
        // Проверка товаров
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM products WHERE (name LIKE ? OR short_description LIKE ?) AND status = 'active'");
        $stmt->execute(["%$query%", "%$query%"]);
        if ($stmt->fetchColumn() > 0) $hasResults = true;
        
        // Проверка услуг
        if (!$hasResults) {
            $stmt = $pdo->prepare("SELECT COUNT(*) FROM services WHERE (name LIKE ? OR short_description LIKE ?) AND is_active = 1");
            $stmt->execute(["%$query%", "%$query%"]);
            if ($stmt->fetchColumn() > 0) $hasResults = true;
        }
...
if (strlen($query) >= 2) {
    // Поиск товаров
    $stmt = $pdo->prepare("
        SELECT id, name, short_description as content, base_price as price 
        FROM products 
    ...
    // Поиск услуг
    $stmt = $pdo->prepare("
        SELECT id, name, short_description as content, price 
        FROM services 
...
$totalResults = count($products) + count($services) + count($news);
?>
<!DOCTYPE html>
<html lang="ru">
<head>
    ...
    <div class="search-page">
        ...
        <?php else: ?>
            ...
            <?php if (!empty($products)): ?>
                <div class="search-section">
                    <div class="search-section-title">
                        <i class="fas fa-box"></i> Товары (<?= count($products) ?>)
                    </div>
                    <div class="search-results">
                        <?php foreach ($products as $item): ?>
                            <div class="search-item">
                                <div class="search-item-type"><i class="fas fa-tag"></i> Товар</div>
                                ...
                                <div class="search-item-price"><?= number_format($item['price'], 0, '.', ' ') ?> ₽</div>
                            </div>
                        <?php endforeach; ?>
                    </div>
                </div>
            ...
            <?php if (!empty($services)): ?>
                <div class="search-section">
                    <div class="search-section-title">
                        <i class="fas fa-concierge-bell"></i> Услуги (<?= count($services) ?>)
                    </div>
                    <div class="search-results">
                        <?php foreach ($services as $item): ?>
                            <div class="search-item">
                                <div class="search-item-type"><i class="fas fa-cogs"></i> Услуга</div>
                                <h3><a href="services_catalog.php?id=<?= $item['id'] ?>"><?= htmlspecialchars($item['name']) ?></a></h3>
                                <p><?= htmlspecialchars(mb_strimwidth(strip_tags($item['content']), 0, 150, '...')) ?></p>
                                <div class="search-item-price">от <?= number_format($item['price'], 0, '.', ' ') ?> ₽</div>
                            </div>
                        <?php endforeach; ?>
                    </div>
                </div>
