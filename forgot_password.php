    ...
    if (empty($email)) {
        ...
        try {
            ...
            if (!$user) {
                ...
                $stmt = $pdo->prepare("UPDATE user SET reset_token = ?, reset_token_expires = ? WHERE id = ?");
                ...
                $subject = "Восстановление пароля на сайте МИП «НПЦ ПИТиА»";
                $htmlMessage = '
                <!DOCTYPE html>
                <html>
                ...
                <body style="font-family: Arial, sans-serif; background: #f5f5f5; padding: 20px;">
                    <div style="max-width: 600px; margin: 0 auto; background: white; border-radius: 10px; padding: 30px;">
                        <h2 style="color: #00a896;">Восстановление пароля</h2>
                        <p>Здравствуйте, <strong>' . htmlspecialchars($user['name']) . '</strong>!</p>
                        <p>Вы запросили восстановление пароля на сайте <strong>ООО МИП «НПЦ ПИТиА»</strong>.</p>
                        <p>Для установки нового пароля нажмите на кнопку ниже:</p>
                        <p style="text-align: center; margin: 30px 0;">
                            <a href="' . $resetLink . '" style="display: inline-block; padding: 12px 30px; background: #00a896; color: white; text-decoration: none; border-radius: 8px;">Сбросить пароль</a>
                        </p>
                ...
                if (sendEmailNotification($user['email'], $user['name'], $subject, $htmlMessage)) {
                    $success = 'На почту ' . htmlspecialchars($email) . ' отправлена ссылка для восстановления пароля. Ссылка действительна 1 час.';
                    $email = '';
                } else {
