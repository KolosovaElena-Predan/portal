...
if (isset($_POST['save'])) {
    ...
    if (!$name) {
        ...
        try {
            ...
            // 1. Сохраняем товар
            $stmt = $pdo->prepare("INSERT INTO products (category_id, name, short_description, full_description, base_price, stock, is_new, is_slider, sort_order, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
                    ...
                    if ($_FILES['product_files']['error'][$k] === UPLOAD_ERR_OK) {
                        ...
                        if (move_uploaded_file($tmp, $fileDir . $fName)) {
                            $pdo->prepare("INSERT INTO product_files (product_id, group_name, file_name, file_url, file_size, sort_order) VALUES (?, ?, ?, ?, ?, ?)")->execute([
                                $productId,
                                trim($_POST['file_group'][$k] ?? 'Документация'),
                                $_FILES['product_files']['name'][$k],
                                'files/products/' . $fName,
                                $_FILES['product_files']['size'][$k],
                                (int)($_POST['file_sort'][$k] ?? 0)
                            ]);
                        }
                    }
                }
                    ...
                    if ($_FILES['scheme_images']['error'][$k] === UPLOAD_ERR_OK) {
                        ...
                        if (move_uploaded_file($tmp, $schemeDir . $sName)) {
                            $pdo->prepare("INSERT INTO product_schemes (product_id, title, description, image_url, sort_order) VALUES (?, ?, ?, ?, ?)")->execute([
                                $productId,
                                trim($_POST['scheme_title'][$k] ?? ''),
                                trim($_POST['scheme_description'][$k] ?? ''),
                                'img/schemes/' . $sName,
                                (int)($_POST['scheme_sort'][$k] ?? 0)
                            ]);
                        }
                    }
                }
                                    ...
                                    if (!empty($v['properties']) && is_array($v['properties'])) {
                                        foreach ($v['properties'] as $p) {
                                            if (!empty($p['name']) && isset($p['price'])) {
                                                $props[] = ['name' => $p['name'], 'price' => (float)$p['price']];
                                            }
                                        }
                                    }
                                    $variants[] = [
...
<?php if ($success && $productId): ?>
<div class="alert alert-success"><?= e($success) ?> <a href="../mip/product.php?id=<?= $productId ?>" target="_blank">Открыть</a></div>
<?php endif; ?>
...
<div class="card-body">
        ...
        <div class="col-md-4">
            <div class="form-section">
                ...
                <select name="category_id" class="form-control">
                    ...
                    <option value="<?= $c['id'] ?>" <?= $c['id'] == $formData['category_id'] ? 'selected' : '' ?>><?= e($c['name']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="form-section">
            ...
            <!-- ✅ Простые чекбоксы без custom-control -->
            <div class="form-section">
        ...
        <div class="col-12">
            ...
            <div class="row" id="imagesContainer">
                ...
                <div class="col-md-3 mb-3 image-card" data-temp-id="<?= e($img['temp_id']) ?>">
                    <div class="card">
                        <img src="../mip/<?= e($img['image_url']) ?>" class="card-img-top" style="height:150px;object-fit:cover;">
                        <div class="card-body p-2">
                            <div class="form-check mb-2">
                                <!-- ✅ Уникальный id для каждой радио-кнопки -->
                                <input type="radio" name="main_image_radio" class="form-check-input main-radio" value="<?= e($img['temp_id']) ?>" id="main_<?= e($img['temp_id']) ?>_<?= uniqid() ?>" <?= ($img['temp_id'] === $mainImageTempId) ? 'checked' : '' ?>>
                                <label class="form-check-label" for="main_<?= e($img['temp_id']) ?>_<?= uniqid() ?>">📌 Главное</label>
                            </div>
                            <button type="button" class="btn btn-sm btn-danger btn-block remove-image" data-temp-id="<?= e($img['temp_id']) ?>"><i class="fas fa-trash"></i></button>
                        </div>
                    </div>
                </div>
                <?php endforeach; ?>
        ...
        <div id="filesContainer">
            <div class="file-entry mb-3 p-3 border rounded">
                <div class="row align-items-end">
                    <div class="col-md-4"><label>Группа</label><input type="text" name="file_group[]" class="form-control" value="Документация"></div>
                    <div class="col-md-5"><label>Файл</label><input type="file" name="product_files[]" class="form-control" accept=".pdf,.doc,.docx,.zip"></div>
                    <div class="col-md-2"><label>Порядок</label><input type="number" name="file_sort[]" class="form-control" value="0" min="0"></div>
    ...
    <div class="row mt-4"><div class="col-12"><h5>Схемы</h5>
        <div id="schemesContainer">
            <div class="scheme-entry mb-3 p-3 border rounded">
                <div class="row">
                    <div class="col-md-4"><label>Название</label><input type="text" name="scheme_title[]" class="form-control" placeholder="Схема подключения"></div>
                    <div class="col-md-4"><label>Изображение</label><input type="file" name="scheme_images[]" class="form-control" accept="image/*"></div>
                ...
                <div class="mt-2"><label>Описание</label><textarea name="scheme_description[]" class="form-control" rows="2" placeholder="Краткое пояснение"></textarea></div>
            </div>
        </div>
        ...
        <div id="configsContainer">
            <div class="config-entry mb-3 p-3 border rounded">
                <div class="row">
                    ...
                    <div class="col-md-5"><label>Характеристики (Ключ: Значение, каждая с новой строки)</label><textarea name="config_chars[]" class="form-control" rows="3" placeholder="Мощность: 1000 Вт&#10;Напряжение: 220В"></textarea></div>
                ...
                <div class="mt-2"><div class="form-check"><input class="form-check-input" type="checkbox" name="config_main[]" value="1"><label class="form-check-label">Основная</label></div></div>
            </div>
        </div>
        <button type="button" class="btn btn-secondary btn-sm" id="addConfigBtn"><i class="fas fa-plus"></i> Добавить комплектацию</button>
        <!--<small class="text-muted d-block mt-1">Формат: <code>Ключ: Значение</code> (каждая пара с новой строки)</small>-->
    </div></div>
    ...
    <div class="row mt-4"><div class="col-12"><h5>Модификации</h5>
        <div id="modsContainer">
            <div class="mod-entry mb-3 p-3 border rounded" data-gidx="0">
                <div class="row">
                    <div class="col-md-4"><label>Название группы</label><input type="text" name="mod_group_name[]" class="form-control" placeholder="Количество входов"></div>
        ...
        <small class="text-muted d-block mt-1">Каждый вариант может иметь доп. свойства с наценкой</small>
    </div></div>
</div>
<div class="card-footer">
    <!-- ✅ Обычное сохранение -->
    <button type="submit" name="save" value="default" class="btn btn-primary">
        <i class="fas fa-plus"></i> Добавить товар
    </button>
    
    <!-- ✅ Сохранение с редиректом (добавлен name="save") -->
    <button type="submit" name="save" value="open_preview" formnovalidate class="btn btn-success">
        <i class="fas fa-external-link-alt"></i> Сохранить и открыть
    </button>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Модификации: варианты и свойства
    document.body.addEventListener('click', function(e) {
...
function addVariant(btn) {
    ...
    const html = `<div class="variant-item card mb-2"><div class="card-body">
        <div class="row align-items-end">
            <div class="col-md-3"><label>Название *</label><input type="text" name="mod_variants[${gidx}][${vidx}][name]" class="form-control form-control-sm" required></div>
            <div class="col-md-2"><label>Цена</label><input type="number" step="0.01" name="mod_variants[${gidx}][${vidx}][price]" class="form-control form-control-sm" value="0"></div>
            <div class="col-md-4"><label>Описание</label><input type="text" name="mod_variants[${gidx}][${vidx}][description]" class="form-control form-control-sm"></div>
            <div class="col-md-2"><button type="button" class="btn btn-danger btn-sm remove-variant-btn">✕</button></div>
            <div class="col-md-1"><button type="button" class="btn btn-sm btn-secondary add-prop-btn">+ Свойство</button></div>
        </div>
        <div class="properties-list mt-2"></div>
    </div></div>`;
...
function addProperty(btn) {
    ...
    const html = `<div class="prop-item input-group input-group-sm mb-1">
        <input type="text" name="mod_variants[${gidx}][${vidx}][properties][${pidx}][name]" class="form-control" placeholder="Свойство">
        <input type="number" step="0.01" name="mod_variants[${gidx}][${vidx}][properties][${pidx}][price]" class="form-control" style="width:90px" placeholder="Наценка">
                    ...
                    if (data.success && data.images.length) {
                        ...
                        data.images.forEach((img, i) => {
                            ...
                            const html = `<div class="col-md-3 mb-3 image-card" data-temp-id="${img.temp_id}">
                                <div class="card"><img src="../mip/${img.image_url}" class="card-img-top" style="height:150px;object-fit:cover;">
                                <div class="card-body p-2"><div class="form-check mb-2">
                                    <input type="radio" name="main_image_radio" class="form-check-input main-radio" value="${img.temp_id}" id="${radioId}" ${isFirst ? 'checked' : ''}>
                                    <label class="form-check-label" for="${radioId}">📌 Главное</label></div>
                                    <button type="button" class="btn btn-sm btn-danger btn-block remove-image" data-temp-id="${img.temp_id}"><i class="fas fa-trash"></i></button></div></div></div>`;
