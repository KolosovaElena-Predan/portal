                    ...
                    <table id="newsTable" class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                ...
                                <th>Заголовок</th>
                                <th style="width: 90px;">Раздел</th> <!-- 👈 Новая колонка -->
                        ...
                        <tbody>
                            <?php if (empty($news)): ?>
                                <tr><td colspan="7" class="text-center text-muted">Новостей пока нет</td></tr>
                            <?php else: ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="news_edit.php?id=<?= $n['id'] ?>" class="btn btn-sm btn-primary" title="Редактировать"><i class="fas fa-edit"></i></a>
                                        <!-- 👈 Динамическая ссылка на сайт -->
                                        <a href="../<?= e($n['section']) ?>/news_view.php?id=<?= $n['id'] ?>" target="_blank" class="btn btn-sm btn-success" title="Открыть на сайте"><i class="fas fa-eye"></i></a>
                                        <a href="news_delete.php?id=<?= $n['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Удалить новость и все изображения?')" title="Удалить"><i class="fas fa-trash"></i></a>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                            <?php endif; ?>
